...
public class EncryptorAesGcm {
    ...
    /* public static String decryptField(byte[] cText, SecretKey secret) throws Exception {
        Cipher cipher = Cipher.getInstance(ENCRYPT_ALGO);
        cipher.init(Cipher.DECRYPT_MODE, secret, new GCMParameterSpec(TAG_LENGTH_BIT, iv));
    ...
    public static String encryptField(String field) {
        ...
        try {
            ...
            byte[] encryptedFieldBytes = encryptWithPrefixIV(field.getBytes(UTF_8), secretKey, iv);
            System.out.println(encryptedFieldBytes.length);
            encryptedField = Base64.getEncoder().encodeToString(encryptedFieldBytes);
    ...
    public static String decryptField(String encryptedField) {
        String field = "";
        try {
            byte[] encryptedFieldBytes = Base64.getDecoder().decode(encryptedField);
            ByteBuffer bb = ByteBuffer.wrap(encryptedFieldBytes);
            ...
            // get AES 256 bits (32 bytes) key
            SecretKey secretKey = CryptoUtils.getAESKey(AES_KEY_BIT);
            field = decrypt(cipherText, secretKey, iv);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return field;
    ...
    public static void main(String[] args) throws Exception {
        ...
        String myPass = encryptField("Felicite1!");
        System.out.println(myPass);
        System.out.println();
        myPass = decryptField(myPass);
        System.out.println(myPass);
        System.out.println();
        String OUTPUT_FORMAT = "%-30s:%s";

        // String pText = "Hello World AES-GCM, Welcome to Cryptography!";
        String pText = "Felicite1!";
